...
//424D 4690 0000 0000 0000 4600 0000 2800 0000 8000 0000 9000 0000 0100
    0x42 /*'B'*/, 0x4d /*'M'*/, 0x00, 0x70, 0x09, 0x00 /*3-6 size of file*/, 0x00, 0x00, 0x00, 0x00 /*7-10 reserved*/,
    0x36, 0x00, 0x00, 0x00 /*11-14 offset to data*/, 0x28, 0x00, 0x00, 0x00 /*15-18 size of info*/,
    0xe0, 0x01, 0x00, 0x00 /*19-22 width*/, 0x40, 0x01, 0x00,0x00 /*23-26 height*/, 0x01, 0x00 /*dimesion always 1*/
};

unsigned char bmp_info_header[] = {
    0x20, 0x00 /*29-30 pixel bit*/, 0x00, 0x00, 0x00, 0x00 /*31-34 compress*/, 0x00, 0x60, 0x09, 0x00/*35-38 size of data*/,
...
void SaveBMPFile(char *file_name, char *buf, int size)
{
    ...
    printf ("pos = %ld\n", ftell(file));
    nread = fwrite(buf, size, size, file);
    //printf ("nread = %d\n", nread);
    //printf ("size = %d\n", size);
    /*if (nread != size)
    {
